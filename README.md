...
- **Atomic batch operations** — grouped file operations with journal rollback (`complete`/`partial`/`failed`); recover-on-error in-process, not crash-durable. A directory is not part of an atomic batch (`create_dir` is rejected); use `create_directory` or a non-atomic batch, then atomic file ops. Opt-in retries: `operation_id` + `retry_contract:"e3-v1"` (persist across restart with `--receipt-dir`)
...
| `apply_patch` | Unified diff. One file or in-process multi-file transaction. `dry_run` + `expected_hash` (single-file). Dest EOL wins. **strict** | Surgical edit. If `PATCH_FAILED`: `read_file` and regenerate; do not retry the same patch |
