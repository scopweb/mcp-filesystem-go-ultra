...
func registerBatchTools(reg *toolRegistry) {
	...
	batchOpsTool := mcp.NewTool("batch_operations",
		...
		mcp.WithObject("request", mcp.Description("Native batch request. Fields: operations (array; types write, edit, search_and_replace, copy, move, delete, create_dir, extract), atomic (bool; recover on error, not crash-durable), create_backup (bool), validate_only (bool), operation_id + retry_contract:\"e3-v1\" (opt-in, 24h receipts; prefix from this receipt namespace; persist across restart with --receipt-dir). extract fields: source, destination, start_line, end_line, append (bool). atomic create_dir is rejected (directories are not journaled; use create_directory or a non-atomic batch). Legacy adapter: request_json."), mcp.AdditionalProperties(true)),
		mcp.WithString("request_json", mcp.Description("Legacy JSON-string adapter for request. Prefer the native request object.")),
		mcp.WithObject("pipeline", mcp.Description("Native pipeline. Fields: name, steps, and optional flags (dry_run, force, stop_on_error, create_backup, verbose, parallel, operation_id, retry_contract:\"e3-v1\" persist with --receipt-dir). Rollback is journal-based (complete/partial/failed); it does not blindly restore backups over later writers. Legacy adapter: pipeline_json."), mcp.AdditionalProperties(true)),
		mcp.WithString("pipeline_json", mcp.Description("Legacy JSON-string adapter for pipeline. Prefer the native pipeline object.")),
		mcp.WithObject("rename", mcp.Description("Native batch rename. Fields: path, mode, find, replace, prefix, suffix, pattern, extension, start_number, padding, recursive, file_pattern, preview, case_sensitive. Legacy adapter: rename_json."), mcp.AdditionalProperties(true)),
		mcp.WithString("rename_json", mcp.Description("Legacy JSON-string adapter for rename. Prefer the native rename object.")),
	)
