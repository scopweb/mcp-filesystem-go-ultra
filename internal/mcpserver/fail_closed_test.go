...
func TestFailClosed_Binary(t *testing.T) {
	...
	// Build the command, not this package: fail-closed is a property of the
	// process entry point, and this package is a library (go build on it would
	// emit an object file, not a runnable binary).
	build := exec.Command("go", "build", "-o", exe, "../../cmd/filesystem-ultra")
