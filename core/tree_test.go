...
func TestListDirectoryTree_HiddenCount(t *testing.T) {
	root := t.TempDir()
	_ = os.WriteFile(filepath.Join(root, ".gitignore"), []byte("secret.txt\n"), 0644)
	_ = os.WriteFile(filepath.Join(root, "ok.go"), []byte("x"), 0644)
	_ = os.WriteFile(filepath.Join(root, "secret.txt"), []byte("nope"), 0644)
	...
	got, err := engine.ListDirectoryTreeResult(context.Background(), root, TreeOpts{
		MaxDepth: 2, MaxNodes: 50, RespectIgnore: true, Format: "compact",
	})
	if err != nil {
		t.Fatal(err)
	}
	if got.HiddenCount < 1 {
		t.Fatalf("hidden_count=%d want >=1\n%s", got.HiddenCount, got.Text)
	}
	if strings.Contains(got.Text, "secret.txt") {
		t.Fatalf("gitignore should hide secret.txt:\n%s", got.Text)
	}
}
